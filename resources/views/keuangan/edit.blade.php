@extends('layouts.app')
@section('title', 'Edit Surat Masuk — Bagian Protokol dan Komunikasi Pimpinan')
...
<div class="form-card">
    <div class="form-header">
        ...
        <p>Perbarui informasi surat masuk, disposisi, berkas lampiran, atau status pencetakan.</p>
    </div>
    ...
    <form method="POST" action="{{ route('keuangan.update', $keuangan->id) }}" enctype="multipart/form-data">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="date" name="tanggal_diterima" class="form-input @error('tanggal_diterima') is-invalid @enderror" value="{{ old('tanggal_diterima', ($keuangan->tanggal_diterima ? $keuangan->tanggal_diterima->format('Y-m-d') : ($keuangan->tanggal ? $keuangan->tanggal->format('Y-m-d') : ''))) }}" required>
            ...
            <div class="form-group">
                ...
                <input type="text" name="nomor_surat" class="form-input @error('nomor_surat') is-invalid @enderror" value="{{ old('nomor_surat', $keuangan->nomor_surat ?? $keuangan->no_bukti) }}" placeholder="Contoh: 180-Bag.Tapem/2026" required>
                @error('nomor_surat')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="form-group">
            ...
            <input type="text" name="pengirim" class="form-input @error('pengirim') is-invalid @enderror" value="{{ old('pengirim', $keuangan->pengirim ?? $keuangan->penanggung_jawab) }}" placeholder="Contoh: Bagian Tata Pemerintahan Setda" required>
            @error('pengirim')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="perihal" class="form-textarea @error('perihal') is-invalid @enderror" placeholder="Tuliskan ringkasan isi atau perihal surat masuk..." required>{{ old('perihal', $keuangan->perihal ?? $keuangan->uraian) }}</textarea>
            @error('perihal')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" name="disposisi" list="disposisiList" class="form-input @error('disposisi') is-invalid @enderror" value="{{ old('disposisi', $keuangan->disposisi) }}" placeholder="Contoh: Suhendro Drajad, S.T. / Andre Pratama, S.I.Kom, M.Si.">
        ...
        <div class="form-group">
            ...
            {{-- Badge file saat ini --}}
            @if($keuangan->file_dokumen || $keuangan->file_bukti)
                @php
                    $currentFileName = basename($keuangan->file_dokumen ?: $keuangan->file_bukti);
                @endphp
                <div id="current-file-badge" class="current-file-badge">
                    ...
                    <span>File saat ini: <strong><a href="{{ $keuangan->file_url }}" target="_blank" style="color:#1e40af;text-decoration:underline">{{ $currentFileName }}</a></strong></span>
                </div>
            @endif

            {{-- Badge file baru terpilih --}}
            <div id="selected-file-badge" class="current-file-badge" style="display:none;background:#f0fdf4;border-color:#bbf7d0;color:#166534">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="width:16px;height:16px;color:#16a34a"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                <span>File baru terpilih: <strong id="selected-file-name">-</strong></span>
                <button type="button" onclick="removeSelectedFile()" style="margin-left:auto;background:none;border:none;color:#dc2626;cursor:pointer;font-size:18px;line-height:1;font-weight:bold" title="Batalkan file baru">&times;</button>
            </div>
        ...
        <div class="form-group" style="margin-top: 10px;">
            ...
            <label class="print-status-box" for="is_printed">
                <input type="checkbox" name="is_printed" id="is_printed" value="1" {{ old('is_printed', $keuangan->is_printed) ? 'checked' : '' }}>
...
function handleFile(input) {
    ...
    if (file) {
        ...
        if (mainText) mainText.textContent = 'File baru berhasil dipilih';
    } else {
        if (badge) badge.style.display = 'none';
        if (mainText) mainText.textContent = 'Drag & Drop file di sini untuk mengganti';
    }
}
