...
<div class="arsip-page">
    ...
    <div class="ar-header-row">
        ...
        <button type="button" class="ar-btn-unggah" onclick="openArsipModal()">
        ...
        <div class="ar-table-responsive">
            <table class="ar-table">
                ...
                <tbody>
                    ...
                    @endphp
                    <tr class="ar-clickable-row" onclick="previewArsip('{{ $fileUrl }}', '{{ addslashes($item->judul) }}', '{{ addslashes($item->nomor_arsip ?? '—') }}', '{{ strtoupper($ext) }}', '{{ addslashes($item->file_name ?? $item->judul) }}', '{{ $formattedDate }}', '{{ addslashes($uploaderName) }}')">
                        {{-- No --}}
                        <td class="ar-cell-no">
                    ...
                    <tr>
                        <td colspan="6" style="text-align:center; padding: 48px 20px; color: #94a3b8;">
                            ...
                            <button type="button" onclick="openArsipModal()" style="color:#2563eb; background:none; border:none; cursor:pointer; font-size:12.5px; font-weight:600;">+ Unggah Dokumen Sekarang</button>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
{{-- ============================================= --}}
<div class="ar-modal-backdrop" id="arsipModal">
    <div class="ar-modal-card">
        <div class="ar-modal-header">
            <div>
                <h2 class="ar-modal-title">Unggah Dokumen Baru</h2>
                <p class="ar-modal-sub">Tambahkan arsip surat atau dokumen baru ke sistem.</p>
            </div>
            <button type="button" class="ar-modal-close" onclick="closeArsipModal()">&times;</button>
        </div>
        <form method="POST" action="{{ route('arsip.store') }}" enctype="multipart/form-data">
            ...
            <div class="ar-modal-body">
                <div class="ar-form-row">
                    <div class="ar-form-g">
                        <label class="ar-form-lbl">Nomor Surat</label>
                        <input type="text" name="nomor_arsip" class="ar-form-input" placeholder="B-123/PROK/2023">
                    </div>
                    <div class="ar-form-g">
                        <label class="ar-form-lbl">Jenis / Kategori Surat <span class="req">*</span></label>
                        <select name="kategori" class="ar-form-select" required>
                ...
                <div class="ar-form-g">
                    <label class="ar-form-lbl">Nama Dokumen / Perihal <span class="req">*</span></label>
                    <input type="text" name="judul" class="ar-form-input" placeholder="Masukkan perihal atau nama dokumen..." required>
                </div>

                <div class="ar-form-row">
                    <div class="ar-form-g">
                        <label class="ar-form-lbl">Tanggal Surat</label>
                        <input type="date" name="tanggal_dokumen" class="ar-form-input" value="{{ date('Y-m-d') }}">
                    </div>
                    <div class="ar-form-g">
                        <label class="ar-form-lbl">Tahun</label>
                        <input type="text" name="tahun" class="ar-form-input" value="{{ date('Y') }}" maxlength="4">
                    </div>
                </div>

                <div class="ar-form-g">
                    <label class="ar-form-lbl">File Dokumen <span class="req">*</span></label>
                    <input type="file" name="file" class="ar-form-input" required accept=".pdf,.doc,.docx,.xls,.xlsx,.zip,.jpg,.jpeg,.png">
                    <div style="font-size:11.5px; color:#94a3b8; margin-top:4px;">Format: PDF, DOC, DOCX, XLS, XLSX, Gambar (Maks. 20MB)</div>
                </div>

                <div class="ar-form-g">
                    <label class="ar-form-lbl">Keterangan / Deskripsi</label>
                    <textarea name="deskripsi" class="ar-form-textarea" rows="2" placeholder="Catatan tambahan mengenai dokumen ini..."></textarea>
                </div>
            </div>
            <div class="ar-modal-footer">
                <button type="button" class="ar-btn-cancel" onclick="closeArsipModal()">Batal</button>
                <button type="submit" class="ar-btn-submit">Simpan &amp; Unggah</button>
            </div>
        </form>
    </div>
</div>
...
// ==========================
function previewArsip(fileUrl, judul, nomor, fileExt, fileName, tanggal, uploader) {
    if (!fileUrl) return;
...
function openArsipModal() {
    document.getElementById('arsipModal').classList.add('is-open');
    document.body.style.overflow = 'hidden';
}

function closeArsipModal() {
    document.getElementById('arsipModal').classList.remove('is-open');
    document.body.style.overflow = '';
}

document.getElementById('arsipModal').addEventListener('click', function(e) {
    if (e.target === this) {
        closeArsipModal();
    }
});
