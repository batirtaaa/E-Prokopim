...
.kg-judul-sub { font-size: 11.5px; color: #9ca3af; margin-top: 2px; }
...
<form method="GET" action="{{ route('kegiatan-pimpinan.index') }}" class="kg-toolbar">
    ...
    {{-- Filter icon --}}
    <button type="button" class="kg-icon-btn" title="Filter">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6h9.75M10.5 6a1.5 1.5 0 11-3 0m3 0a1.5 1.5 0 10-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-9.75 0h9.75"/></svg>
    </button>
    {{-- Export icon --}}
    <button type="button" class="kg-icon-btn" title="Export">
    ...
    <table class="kg-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                {{-- Judul --}}
                <td>
                    <div class="kg-judul">{{ Str::limit($item->judul, 35) }}</div>
                    @if($item->kategori)
                        <div class="kg-judul-sub">{{ ucfirst($item->kategori) }}</div>
                    @endif
                </td>
                ...
                <td>
                    ...
                    <div class="kg-tanggal-time">
                        {{ $item->tanggal_mulai ? $item->tanggal_mulai->format('H:i') : '' }}
                        @if($item->tanggal_selesai) - {{ $item->tanggal_selesai->format('H:i') }} @endif
                        WIB
                    </div>
                </td>
                ...
                <td>
                    <div class="kg-aksi">
                        {{-- WhatsApp --}}
                        <a href="https://wa.me/?text={{ urlencode('Kegiatan: ' . $item->judul . ' | ' . ($item->tanggal_mulai ? $item->tanggal_mulai->format('d M Y H:i') : '') . ' WIB | ' . ($item->lokasi ?? '')) }}"
